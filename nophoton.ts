...
async function loadQuestServers() {
  ...
  if (symResponse.status === 200) {
    try {
      ...
      console.log("[+] Symbols loaded from cloud and re-mapped.");
    } catch (e) {
      console.log("[-] Error applying cloud symbols: " + e);
    }
  } else {
    console.log(
      "[-] Cloud failed (Status: " +
    ...
    try {
      console.log("[+] Fallback symbols applied.");
    } catch (err) {
  ...
  Il2Cpp.perform(() => {
    ...
    console.log("[+] Quest Servers Loaded Successfully!");
    console.log("[+] Made by byte | discord.gg/syte!");
  });
}
